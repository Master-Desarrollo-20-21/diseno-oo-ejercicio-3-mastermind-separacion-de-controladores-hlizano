...
public class SecretCombination extends Combination {
	...
	public SecretCombination() {
		for (int i = 0; i < 4; i++) {
			...
			for(int j = 0; j<i; j++) {
				String colorList = String.valueOf(this.proposedColors.charAt(j));
				while(color.equals(colorList)) {
					color = Color.getRandomColor();
				} 		
			}	
			this.proposedColors += color;
		}
	}
	...
	public Result compareCombination(Combination proposed) {		
		...
		for(int i = 0; i<this.getWidth(); i++) {
			char colorProposed = proposed.getProposedColors().charAt(i);
			for(int j = 0; j<this.getWidth(); j++) {	
				char colorSecret = this.proposedColors.charAt(j);
